...
sub p2p_transfer
{
        ...
        if($P2PHeader->{Flag} == 2){ #got p2p ack
                ...
                if($data =~ m/INVITE MSNMSGR/gs){ #got invite
                    $self->GotInvite($P2PHeader->{Sesid}, $user, $data);
                }
...
sub GotInvite{
    ...
    if($params->{'EUF-GUID:'}){
        ...
        if ($params->{'EUF-GUID:'} eq '{A4268EEC-FEC5-49E5-95C3-F126696BDBF6}'){
                ...
                $self->{$params->{'SessionID:'}}->{FileReply} = $self->{SB}->{Msn}->call_event($self->{SB}, "FileReceive", $user, $self->{$params->{'SessionID:'}}->{FileName}, $self->{$params->{'SessionID:'}}->{FileSize}, $params);
		...
		#$self->Send200OK($sid);
		if( $self->{SB}->{Msn}->call_event( $self->{SB}, "Invitation", $user, $msg ) )
		{
			$self->Send200OK($sid);
		}
		else
