...
sub sendSharedSearch
{
        my $self = shift;
        my $query = shift;

        if( !defined $query ){ return $self->error( "You must pass the query you wish to use" ); }
...
sub sendMessage
{
        ...
        if( length $response ){
                ...
                @emotes = grep { index($response, $_) > -1 } @emotes;
                ...
                my $header = "MIME-Version: 1.0\r\n".
                             ...
                             "X-MMS-IM-Format: FN=$style->{Font}; EF=$style->{Effect}; ".
                             "CO=$style->{Color}; CS=$style->{CharacterSet};PF=$style->{PitchFamily}\r\n";

                $header .= "P4-Context: " . $style->{Name} . "\r\n" if( exists $style->{Name} );
                if($chunks > 1){
                             for (my $chunk = 0; $chunk < $chunks; $chunk++) {
                                  if($chunk == 0){
                                  $header .= "Message-ID: {7B7B34E6-7A8D-44FF-926C-1799156B5805}\r\n".
                                             "Chunks: $chunks\r\n\r\n";
                                  }
                                  else{
                                  $header = "Message-ID: {7B7B34E6-7A8D-44FF-926C-1799156B5805}\r\n".
                                  ...
                                  $header .= substr($response, $chunk * 1400, 1400) . "\r\n";
                                  $self->sendraw('MSG', 'N '.length($header). "\r\n" . $header);
                              };
                }
                else{
...
sub sendRecording
{
        ...
        my $header = qq{MIME-Version: 1.0\nContent-Type: text/x-msmsgscontrol\nRecordingUser: } . $self->{Msn}->{Handle} . qq{\n\n\n};
        $header =~ s/\n/\r\n/gs;
        $self->sendraw( 'MSG', 'N ' . length($header) . "\r\n" . $header);
}
...
print( "CONTENT TYPE: $header->{'Content-Type'}\n" );
        ...
        {
		if( $header->{'RecordingUser'} )
		{
			$self->{Msn}->call_event( $self, "Recording", $user, uri_unescape($friendly) );
		}
		else
		{
	...
	elsif( $header->{'Content-Type'} =~ /application\/x-msnmsgrp2p/ )
	{
		...
		#$self->{Msn}->call_event( $self, "Invitation", $user, $msg );
	}
	elsif( $header->{'Content-Type'} =~ /text\/search-query/ )
	{
		my $format_info = $header->{'X-MMS-IM-Format'};
                $format_info =~ /FN=(.*?);\s+EF=(.*?);\s+CO=(.*?);\s+CS=(.*?);\s+PF=(.*?)/;
                my %style = ( Font => $1, Effect => $2, Color => $3, CharacterSet => $4, PitchFamily => $5 );

		$self->{Msn}->call_event( $self, "Search", $user, uri_unescape($friendly), $msg, %style );
	}
	...
	elsif( $header->{'Content-Type'} =~ /text\/x-msnmsgr-datacast/ )
	{
		#ID: 1 = Nudge
		#ID: 2 = Wink
		#ID: 3 = Sound Clip
	}
	elsif( $header->{'Content-Type'} =~ /text\/x-msmsgsprofile/ )
	{
		#profile info
	}
	...
	{
		$self->{Msn}->call_event( $self, "Search", $user, uri_unescape($friendly), $msg );
	}
