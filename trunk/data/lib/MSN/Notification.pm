...
use constant VER => 'MSNP12 CVR0';
#use constant VER => 'MSNP13 CVR0';
...
sub setPSM
{
	...
	my $data = '<Data><PSM>'.$psm.'</PSM><CurrentMedia></CurrentMedia><LOC>\0Location\01\0Tearios Computer\0Tearios Computer\0\01\0</LOC></Data>';
...
{
	my $self = shift;
	my( $title, $artist, $album ) = @_;

	my $data = '<Data><PSM></PSM><CurrentMedia>\0Music\01\0{0} - {1}\0'.$title.'\0'.$artist.'\0'.$album.'\0\0</CurrentMedia></Data>';
	$self->sendraw("UUX",  length($data)."\r\n" . $data);
}
...
sub setGame
{
	...
	my $data = '<Data><PSM></PSM><CurrentMedia>\0Games\01\0{0}\0'.$game.'\0</CurrentMedia></Data>';
	$self->sendraw("UUX",  length($data)."\r\n" . $data);
}
...
sub setDoc
{
	...
	my $data = '<Data><PSM></PSM><CurrentMedia>\0Office\01\0{0}\0'.$document.'\0</CurrentMedia></Data>';
	$self->sendraw("UUX",  length($data)."\r\n" . $data);
}
...
sub CMD_GCF
{
	...
	#called when the profile information is received
...
sub CMD_OUT
{
	#called when the bot is kicked off the server (if the same account signs in somewhere else)
}

sub CMD_SBS
{
	#information about mobile credits
}

sub CMD_BPR
{
	#received when contact has blog or phone number etc...
	#Values:
	#PHM = Phone Mobile
	#PHW = Phone Work
	#PHH = Phone Home
	#MOB = MSN Mobile device enabled
	#WWE = MSN Direct device enabled
	#HSB = MSN Spaces Blog
}

sub CMD_PRP
{
	#received when display name is updated
}
...
sub CMD_MSG
{
	...
	if( $msg =~ /text\/x-msmsgsprofile/ )
	{
		$self->{Msn}->call_event( $self, 'Profile', $header, $msg );
	}
	elsif( $msg =~ /text\/x-msmsgsinitialmdatanotification/ )
	{
		$self->{Msn}->call_event( $self, 'Email', $header, $msg );
	}
	elsif( $msg =~ /text\/x-msmsgsoimnotification/ )
	{
		$self->{Msn}->call_event( $self, 'OIM', $header, $msg );
	}
	elsif( $msg =~ /text\/x-msmsgsinitialemailnotification/ )
	{
		$self->{Msn}->call_event( $self, 'Email', $header, $msg );
	}
	elsif( $msg =~ /text\/x-msmsgsemailnotification/ )
	{
		$self->{Msn}->call_event( $self, 'Email', $header, $msg );
	}
	elsif( $msg =~ /text\/x-msmsgsactivemailnotification/ )
	{
		$self->{Msn}->call_event( $self, 'Email', $header, $msg );
	}
}
...
sub CMD_UBX
{
	...
	$self->{Msn}->call_event( $self, 'PSM', $email, $msg );
}

sub CMD_UUX
{
	#received when PSM is set successfully
}
...
sub CMD_CHL
{
	...
	#$self->sendraw( 'QRY', 'PROD01065C%ZFN6F' . "\r\n" . $qryhash ); (P13)
}
...
use Math::BigInt;
	...
	#my $prodkey = shift || 'O4BG@C7BWLYQX?5G'; (P13)
