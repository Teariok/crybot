...
sub convertFromCid
{
	...
	if( $cid >= 1610612736 )		{ $info->{Client} = 'MSNC6'; }
	elsif( $cid >= 1342177280 )		{ $info->{Client} = 'MSNC5'; }
	elsif( $cid >= 1073741824 )		{ $info->{Client} = 'MSNC4'; }
	...
	$info->{WebMessenger} = ($cid & 512) ? 1 : 0;
	$info->{InternalClient} = ($cid & 2048) ? 1 : 0;
	$info->{UnknownTwo} = ($cid & 4096) ? 1 : 0;
	$info->{DirectIM} = ($cid & 16384) ? 1 : 0;
	$info->{Winks} = ($cid & 32768) ? 1 : 0;
	$info->{SharedSearch} = ($cid & 65536) ? 1 : 0;
	$info->{VoiceClips} = ($cid & 262144) ? 1 : 0;
	$info->{VoIP} = ($cid & 524288) ? 1 : 0;
	$info->{UnknownThree} = ($cid & 16777216) ? 1 : 0;
	$info->{SharingFolders} = ($cid & 4194304) ? 1 : 0;
...
sub convertToCid
{
	...
	elsif( $info{Client} eq 'MSNC6' )	{ $cid = 1610612736; }
	elsif( $info{Client} eq 'MSNC5' )	{ $cid = 1342177280; }
	elsif( $info{Client} eq 'MSNC4' )	{ $cid = 1073741824; }
	...
	$cid += 4 if( $info{GifInk} );
	$cid += 8 if( $info{ISFInk} );
	$cid += 16 if( $info{Video} );
	$cid += 32 if( $info{MultiPacket} );
	$cid += 64 if( $info{MSNMobile} );
	$cid += 128 if( $info{MSNDirect} );
	$cid += 512 if( $info{WebMessenger} );
	#$cid += 2048 if( $info{InternalClient} );
	#$cid += 4096 if( $info{UnknownTwo} ); #WLM8, Still Unknown
	$cid += 16384 if( $info{DirectIM} );
	$cid += 32768 if( $info{Winks} );
	$cid += 65536 if( $info{SharedSearch} );
	$cid += 262144 if( $info{VoiceClips} );
	$cid += 524288 if( $info{VoIP} ); #not definate
	#$cid += 16777216 if( $info{UnknownThree} ); #WLM8, Still Unknown
	$cid += 4194304 if( $info{SharingFolders} );
