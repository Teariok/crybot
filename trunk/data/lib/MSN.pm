...
use constant CVER10 => '0x0409 winnt 5.1 i386 MSNMSGR 8.0.0792 MSMSGS ';
...
sub cmdError
{
	...
	$self->call_event( $self, 'CMDError', "UNDEFINED CMD : $message" ) if( $self->{CMDError} );
...
sub actionMessage
{
	...
	return $self->{Notification}->actionMessage( @_ );
}
...
sub setSong
{
	...
	return $self->{Notification}->setSong( @_ );
}
...
sub setGame
{
	...
	return $self->{Notification}->setGame( @_ );
}
...
sub setDoc
{
	...
	return $self->{Notification}->setDoc( @_ );
}
...
Sends a service url request to the notification server
...
{
	my $self = shift;
	return $self->{Notification}->ServiceURL( @_ );
}

=item
promoteByName

Sends a promotional email with the given name to the given email address

=cut
sub promoteByName
{
	my $self = shift;
	return $self->{Notification}->promoteByName( @_ );
}
